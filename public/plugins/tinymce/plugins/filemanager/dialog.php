...
if(isset($_GET['subfolder']) && !empty($_GET['subfolder'])) {
    ...
    $thumbs_path = '/'.$thumbs_dir . $subfolder . '/';
...
<!----- header div end ------->
	...
	<div class="span12">
		...
		foreach ($files as $file) {
			...
			if (!file_exists($abs_root_path.$thumbs_path.$subdir.$file)) create_folder(false,$abs_root_path.$thumbs_path.$subdir.$file);
			$class_ext = 3;			
			if($file=='..' && trim($subdir) != '' ){
			    $src = explode('/',$subdir);
			...
			if ($file != '.' && $file != '..' && !is_dir($root . $cur_dir . $file)) {
				...
				$src = '';
				$src_thumb = $thumbs_path.DIRECTORY_SEPARATOR.$subdir. $file;
				//add in thumbs folder if not exist 
				$thumb_path=$current_path."../thumbs".DIRECTORY_SEPARATOR.$subfolder.DIRECTORY_SEPARATOR.$subdir.$file;
				if(!file_exists($thumb_path)) create_img_gd($current_path.$subfolder.DIRECTORY_SEPARATOR.$subdir.$file, $thumb_path, 122, 91);
				$is_img=true;
				//check if is smaller tha thumb
				$info=getimagesize($current_path.$subfolder.DIRECTORY_SEPARATOR.$subdir.$file);
				if($info[0]<122 && $info[2]<91){
				    $src_thumb= $relative_path.$subfolder.DIRECTORY_SEPARATOR.$subdir.$file;
				    $show_original=true;
				}
